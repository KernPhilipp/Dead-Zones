...
func _ready():
	...
	if current_scene == null:
		push_error("GameManager could not find the current scene.")
		return

	player = get_tree().get_first_node_in_group("player")
	hud = _find_hud(current_scene)
	...
	if hud != null:
		...
		push_warning("GameManager could not find HUD; wave loop was not started.")
...
func _find_hud(current_scene: Node) -> CanvasLayer:
	var scene_hud := current_scene.find_child("HUD", true, false) as CanvasLayer
	if scene_hud != null:
		return scene_hud
	if has_node("../HUD"):
		return get_node("../HUD") as CanvasLayer
	return null
